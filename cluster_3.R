## ============================================================
## C = 3 STUDY (10 setups × 100 datasets each)
## - Panel A: true-label scatter montage (first 12 datasets)
## - Panel B: elbow per dataset (C = 1:8), choose C* by visual knee
## - Histogram of chosen C* per setup
## ============================================================
...
save_elbows   <- FALSE        # TRUE -> write 100 elbow PNGs per setup
scatter_panels_to_show <- 12  # how many scatters in montage (first N)
panel_layout  <- c(3,4)       # rows × cols for scatter montage
out_dir <- "AGM_C3_STUDY"
...
## -------------------- TEN PARAMETRIC SETUPS (true C=3) --------------------
# We vary means, balance, rotation, and covariance shape/size.
setup_list <- list(
  list(name="T1_balanced_triangle_sep_med",
       pis=c(1,1,1)/3,
       mus=rbind(c( 0,  4), c(-3.5,-2), c( 3.5,-2)),
       Sigmas=rep(list(rot_cov(1.0,0.6,0)), 3)),
  list(name="T2_balanced_triangle_sep_large",
       pis=c(1,1,1)/3,
       mus=rbind(c( 0,  5.5), c(-4.5,-2.5), c( 4.5,-2.5)),
       Sigmas=rep(list(rot_cov(1.0,0.6,0)), 3)),
  list(name="T3_balanced_line",
       pis=c(1,1,1)/3,
       mus=rbind(c(-5,0), c(0,0), c(5,0)),
       Sigmas=rep(list(rot_cov(1.0,0.6,0)), 3)),
  list(name="T4_imbalanced_50_30_20",
       pis=c(0.50,0.30,0.20),
       mus=rbind(c( 0,  4), c(-3.2,-2), c( 3.2,-2)),
       Sigmas=rep(list(rot_cov(1.0,0.6,0)), 3)),
  list(name="T5_imbalanced_60_30_10",
       pis=c(0.60,0.30,0.10),
       mus=rbind(c( 0,  4.2), c(-3.4,-2.1), c( 3.4,-2.1)),
       Sigmas=rep(list(rot_cov(1.0,0.6,0)), 3)),
  list(name="T6_rotated_phi30",
       pis=c(1,1,1)/3,
       mus=rbind(c( 0,  4), c(-3.2,-2), c( 3.2,-2)),
       Sigmas=list(rot_cov(1.2,0.5, pi/6),
                   rot_cov(1.0,0.7,-pi/6),
                   rot_cov(1.1,0.6, pi/6))),
  list(name="T7_rotated_phi45_unbalanced",
       pis=c(0.55,0.25,0.20),
       mus=rbind(c( 0,  4), c(-3.0,-2.2), c( 3.5,-2.0)),
       Sigmas=list(rot_cov(1.3,0.5, pi/4),
                   rot_cov(1.0,0.7,-pi/4),
                   rot_cov(1.1,0.6, pi/4))),
  list(name="T8_high_variance_overlap",
       pis=c(1,1,1)/3,
       mus=rbind(c( 0,  3.2), c(-2.8,-1.6), c( 2.8,-1.6)),
       Sigmas=rep(list(rot_cov(1.6,1.0,0)), 3)),
  list(name="T9_eccentric_mismatch",
       pis=c(0.34,0.33,0.33),
       mus=rbind(c( 0,  4.0), c(-3.4,-2.0), c( 3.4,-2.0)),
       Sigmas=list(rot_cov(1.4,0.4,0), rot_cov(0.8,0.6,0), rot_cov(1.1,0.5,0))),
  list(name="T10_close_triangle",
       pis=c(1,1,1)/3,
       mus=rbind(c( 0,  2.6), c(-2.4,-1.2), c( 2.4,-1.2)),
       Sigmas=rep(list(rot_cov(1.2,0.8,0)), 3))
)
...
for (s in seq_along(setup_list)) {
  ...
  for (d in 1:to_show) {
    ...
    plot(Y[,1], Y[,2], col = cols[z], pch = 16, cex = 0.7,
         xlab = "Y1", ylab = "Y2",
         main = sprintf("TRUE C=3 | ds=%d", d))
    grid()
    if (d == 1) legend("topright", legend=c("Comp 1","Comp 2","Comp 3"),
  ...
  abline(v = bp[which(Cs_grid == 3)], col="forestgreen", lty=2, lwd=2)
  mtext("True C = 3", side=3, line=0.2, col="forestgreen")
  dev.off()
